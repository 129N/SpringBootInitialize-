...
public abstract class AbstractService < ID extends Serializable, T extends AbstractDomain<ID> > {
    ...
    @Transactional
    public T save(T entity) {return this.repository.save(entity);}

    @Transactional
    public T save (ID id, T e) throws ResourceNotFoundException{
        T entity = findByID(id).orElseThrow( ()-> new ResourceNotFoundException("Cannot find by id:$s".formatted(id)) );
    ...
    //protected abstract Client copy(Client original, Client modified);
}

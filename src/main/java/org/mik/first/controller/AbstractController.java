...
public abstract class AbstractController<ID extends Serializable, E extends AbstractDomain<ID>> {
    ...
    public ResponseEntity<E> find(@PathVariable("id") ID id) throws Exception {
        return ResponseEntity.ok(this.service.findById(id)
                .orElseThrow(()->new ResourceNotFoundException("Cannot find entity with id:%s".formatted(id))));
    }
